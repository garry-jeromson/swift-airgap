...
extension AllAirgapUnitTests {
    ...
    final class AirgapWebSocketTests {
        ...
        @Test("WebSocket task produces violation") func webSocketTaskProducesViolation() throws {
            Airgap.activate()

            let session = URLSession(configuration: .default)
            let task = try session.webSocketTask(with: #require(URL(string: "wss://example.com/ws")))
            task.resume()
        ...
        @Test("Non-TLS WebSocket task produces violation") func nonTLSWebSocketTaskProducesViolation() throws {
            Airgap.activate()

            let session = URLSession(configuration: .default)
            let task = try session.webSocketTask(with: #require(URL(string: "ws://example.com/ws")))
            task.resume()
        ...
        @Test("WebSocket violation contains URL and method") func webSocketViolationContainsURLAndMethod() throws {
            Airgap.activate()

            let session = URLSession(configuration: .default)
            let task = try session.webSocketTask(with: #require(URL(string: "wss://example.com/chat")))
            task.resume()
        ...
        @Test("WebSocket task not intercepted when inactive") func webSocketTaskNotInterceptedWhenInactive() throws {
            // Don't activate
            let session = URLSession(configuration: .default)
            let task = try session.webSocketTask(with: #require(URL(string: "wss://example.com/ws")))
            task.resume()
        ...
        @Test("WebSocket task respects allowed hosts") func webSocketTaskRespectsAllowedHosts() throws {
        ...
        @Test("WebSocket task is cancelled after violation") func webSocketTaskIsCancelledAfterViolation() async throws {
            ...
            let session = URLSession(configuration: .default)
            let task = try session.webSocketTask(with: #require(URL(string: "wss://example.com/ws-cancel")))
            task.resume()
