...
extension AllAirgapUnitTests {
    ...
    final class AirgapConfigurationTests {
        ...
        @Test("Warn mode does not fail test") func warnModeDoesNotFailTest() async throws {
            Airgap.mode = .warn
            Airgap.activate()

            let url = try #require(URL(string: "https://example.com/api/warn-test"))
        ...
        @Test("Fail mode calls violation handler directly") func failModeCallsViolationHandlerDirectly() async throws {
            Airgap.mode = .fail
            Airgap.activate()

            let url = try #require(URL(string: "https://example.com/api/fail-test"))
            _ = try? await URLSession.shared.data(from: url)
        ...
        @Test("Custom error code is delivered") func customErrorCodeIsDelivered() async throws {
            Airgap.errorCode = NSURLErrorTimedOut
            Airgap.activate()

            let url = try #require(URL(string: "https://example.com/error-code-test"))
            do {
                _ = try await URLSession.shared.data(from: url)
        ...
        @Test("Default error code is not connected to internet") func defaultErrorCodeIsNotConnectedToInternet() async throws {
            Airgap.activate()

            let url = try #require(URL(string: "https://example.com/default-error-test"))
            do {
                _ = try await URLSession.shared.data(from: url)
                Issue.record("Expected an error to be thrown")
            } catch {
        ...
        @Test("Response delay adds latency") func responseDelayAddsLatency() async throws {
            Airgap.responseDelay = 0.5
            Airgap.activate()

            let url = try #require(URL(string: "https://example.com/delay-test"))
            let start = Date()
            _ = try? await URLSession.shared.data(from: url)
        ...
        @Test("Violation reporter receives violation struct") func violationReporterReceivesViolationStruct() async throws {
            ...
            Airgap.activate()
            let url = try #require(URL(string: "https://example.com/reporter-test"))
            _ = try? await URLSession.shared.data(from: url)
        ...
        @Test("Violation reporter called alongside handler") func violationReporterCalledAlongsideHandler() async throws {
            let reporterCapture = ViolationReporterCapture()
            Airgap.violationReporter = { violation in
                reporterCapture.record(violation)
            }
            ...
            Airgap.activate()
            let url = try #require(URL(string: "https://example.com/alongside-test"))
            _ = try? await URLSession.shared.data(from: url)
            await drainMainQueue()
